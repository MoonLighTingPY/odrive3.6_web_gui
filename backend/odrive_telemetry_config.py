...
def get_full_device_state(odrv):
    """Get a complete snapshot of all device properties"""
    if not odrv:
        return {'device': {}}
    
    # Get both high frequency and configuration data
    high_freq_data = get_high_frequency_telemetry(odrv)
    config_data = get_configuration_data(odrv)
    ...
    return merge_dicts(high_freq_data, config_data)
